    ...
    <b-container fluid>
      ...
      <b-card-group columns style="padding-top:10px">
        ...
        <b-card>
          <myecharts
            :theid="chartName"
            :thedata="wholeAttendance"
            :theseries="series"
            v-if="isshow"
          >
            <p>今日迟到人员:</p>
            <span v-for="item in todayChidao" :key="item.id">{{item.namePer}},</span>
            <p>今日早退人员:</p>
            <span v-for="item in todayZaotui" :key="item.id">{{item.namePer}},</span>
          </myecharts>
          <template #footer>
        ...
        <b-card header="说点什么">
          <blockquote class="blockquote mb-0">
            ...
            <footer class="blockquote-footer">源于朱自清</footer>
          </blockquote>
        </b-card>
...
export default {
  ...
  methods: {
    ...
    deleteFireTime: function() {
      this.$bvModal
        .msgBoxConfirm("确认清空该员工离职时间吗？当清空后请点击确认才会生效！")
        .then(value => {
          this.boxOne = value;
  ...
  created() {
    ...
    todayDelay(todayTime, this.part).then(res => {
      ...
      for (let i = 0; i < res.data.length; ++i) {
        if (res.data[i].clockState === "迟到") {
          index++;
          let form = {};
          form = res.data[i];
          this.todayChidao.push(form);
        } else {
          index1++;
          let form = {};
          form = res.data[i];
          this.todayZaotui.push(form);
        }
        // console.log(this.todayChidao);
      }
      for (let i = 0; i < res.data.length; ++i) {
