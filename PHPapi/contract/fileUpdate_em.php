...
$c=json_decode($v,true);
   //如果他不是空的则运行这条sql
    ...
    $file_path="$file_location$file_name";
     ...
     //获取到地址删除掉
$sql="select `src` from `personnel` where id={$id}";
...
while ($row = $result -> fetch_assoc()) {
  ...
  // var_dump($src);
       $uploadSql=" UPDATE `personnel` SET `uploaduser`='{$username}',`partPer`='{$part}',`fileName`='{$file_name}',`uploadTime`='{$uploadTime}', `tip`='{$fileContent}',`src`='{$src}' WHERE id={$id} ";
       ...
       $fileContent=$c['tips'];
       $username=$c['username'];
       $uploadSql=" UPDATE `personnel` SET `uploaduser`='{$username}',`partPer`='{$part}', `tip`='{$fileContent}' WHERE id={$id} ";
       $result = $mysqli -> query($uploadSql);
    //    var_dump($uploadSql);
}
